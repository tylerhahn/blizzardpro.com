...
if (typeof theme !== "undefined" && theme.hasOwnProperty("settings"))
  theme.settings.predictiveSearchEnabled = false;
// Override Settings
var boostPFSFilterConfig = {
  general: {
    ...
    cropImagePossitionEqualHeight:
      boostPFSThemeConfig.custom.equal_height_crop_image_position,
...
(function () {
  ...
  ProductGridItem.prototype.compileTemplate = function (data) {
    ...
    // itemActiveSwapClass
    itemHtml = itemHtml.replace(
      /{{itemActiveSwapClass}}/g,
      buildActiveSwapClass(data)
    );
    // Add Color Swatches
    itemHtml = itemHtml.replace(
      /{{itemColorSwatches}}/g,
    ...
    itemHtml = itemHtml.replace(
      /{{itemUrl}}/g,
      Utils.buildProductItemUrl(data)
    );
    return itemHtml;
  };
  ...
  ProductListItem.prototype.compileTemplate = function (data) {
    ...
    itemDescription =
      itemDescription.split(" ").length > 35
        ? itemDescription.split(" ").splice(0, 35).join(" ") + "..."
        : itemDescription.split(" ").splice(0, 35).join(" ");
  ...
  function buildImages(data) {
    ...
    for (var i = 0; i < rangeWidths.length; i++) {
      dataSrcSet +=
        Utils.getFeaturedImage(images, rangeWidths[i] + "x") +
        " " +
        rangeWidths[i] +
        "w";
    ...
    html +=
      '<img class=" lazyload Image--lazyLoad"' +
      'data-srcset="' +
      dataSrcSet +
      '" ' +
      'data-src="' +
  ...
  function buildVendor(data) {
    var html = "";
    if (
      boostPFSThemeConfig.custom.hasOwnProperty("show_vendor") &&
  ...
  function buildPrice(data) {
    ...
    let priceType;
    let isBstock = false;
    let isDiscForSale = false;
    let variantData;

    function getVariantPrice(data, value, type) {
      priceType = value;
      const option = _.filter(data.variants, { [type]: value });
      if (option.length > 0) {
        variantId = option[0].id;
        variantPrice = option[0].price;
        variantData = option[0];
      }
    }
    if (url.includes("25ft+%281425%29")) {
      getVariantPrice(data, "25ft (1425)", "option_length");
    } else if (url.includes("3ft+%281403%29")) {
      getVariantPrice(data, "3ft (1403)", "option_length");
    } else if (url.includes("opt_length=1.5ft")) {
      getVariantPrice(data, "1.5ft", "option_length");
    } else if (url.includes("opt_length=3ft")) {
      getVariantPrice(data, "3ft", "option_length");
    } else if (url.includes("opt_length=5ft")) {
      getVariantPrice(data, "5ft", "option_length");
    } else if (url.includes("opt_length=6ft")) {
      getVariantPrice(data, "6ft", "option_length");
    } else if (url.includes("opt_length=10ft")) {
      getVariantPrice(data, "10ft", "option_length");
    } else if (url.includes("opt_length=15ft")) {
      getVariantPrice(data, "15ft", "option_length");
    } else if (url.includes("opt_length=25ft")) {
      getVariantPrice(data, "25ft", "option_length");
    } else if (path === "/collections/like-new") {
      isBstock = true;
      getVariantPrice(data, "Like New", "option_quality");
    } else if (path === "/collections/refurbished") {
      isBstock = true;
      getVariantPrice(data, "Refurbished", "option_quality");
    } else if (path === "/collections/discontinued-for-sale") {
      isDiscForSale = true;
    } else {
      variantPrice = data.price_max;
    ...
    function renderVariantSelect() {
      ...
      _.map(variants, (variant, i) => {
        if (
          ...
          str =
            str +
            `<div class='variant_select_bubble ${i === 0 ? "active" : ""}'>${
              variant.option1
            }</div>`;
        }
      });
    ...
    function renderAddToCart() {
      if (data.available) {
        ...
        <form method="post" action="/cart/add">
          ...
          <input type="submit" value="Add to Cart" class="btn btn-blue" />
          <a class="btn btn-ul" style="width: 100px; margin-top: 14px; margin-left: 10px" href='/products/${data.handle}'>Learn More</a>
        </form> `;
      } else {
        return `<a class="btn btn-ul" style="width: 100px; margin-top: 14px; margin-left: 10px" href='/products/${data.handle}'>Learn More</a>`;
      }
    }
    var price = Utils.formatMoney(variantPrice);
    var subtitle;
    var removeDecimal = price.replace("&#36;", "").split(".");
    var html = "";
    const bstockOptions = ["like-new, refurbished"];
    if (data.metafields.length > 0) {
      const categoryFb = _.filter(data.metafields, { namespace: "f_b" });
      const categoryCf = _.filter(data.metafields, { namespace: "c_f" });
      if (categoryFb.length > 0) {
        subtitle = categoryFb[0].value;
      } else if (categoryCf.length > 0) {
        subtitle = categoryCf[0].value;
      }
    }
    if (
      boostPFSThemeConfig.custom.hasOwnProperty("show_price") &&
      boostPFSThemeConfig.custom.show_price
    ) {
      html += `
      ${
        priceType !== undefined
          ? `<h4 class="variant-bubble">${priceType}</h4>`
          : ""
      }
      <a class="boost-pfs-filter-product-item-title" href="/products/${
        data.handle
      }${variantData ? `?variant=${variantData.id}` : ""}">${data.title}</a>
      ...
      <div class="ais-hit--price">
        ...
        ${
          data.compare_at_price_max > 0 &&
          data.compare_at_price_max > variantPrice
  ...
  function buildLabels(data) {
    ...
    var soldOutLabel = "";
    if (
      boostPFSThemeConfig.custom.hasOwnProperty("show_sold_out_label") &&
      boostPFSThemeConfig.custom.show_sold_out_label &&
      soldOut
    ) {
        ...
        if (data.tags) {
          for (var i = 0; i < data.tags.length; i++) {
            ...
            if (tag.indexOf("pfs:label") !== -1) {
              var preTagLabel = boostPFSTemplate.tagLabelHtml.replace(
                /{{labelTag}}/g,
                tag.split("pfs:label-")[1]
              );
  ...
  function buildColorSwatches(data) {
    ...
    if (
      boostPFSThemeConfig.custom.hasOwnProperty("display_item_swatch") &&
      boostPFSThemeConfig.custom.display_item_swatch == true
    ) {
      if (
        typeof data.options_with_values != "undefined" &&
        data.options_with_values.length > 0
      ) {
        ...
        if (swatchArr.length > 0) {
          ...
          for (var sIndex = 0; sIndex < swatchLimit; sIndex++) {
            ...
            if (
              boostPFSThemeConfig.custom.hasOwnProperty(
                "swatch_color_display_type"
              )
            ) {
              switch (boostPFSThemeConfig.custom.swatch_color_display_type) {
                case "swatch_color_display_type_image_color":
                  bgImageSrc = boostPFSAppConfig.general.file_url.replace(
                    /\?/,
                    Utils.slugify(filterSwatchTitle).replace(/\-/g, "_") +
                      "-" +
                      Utils.slugify(swatchName) +
                      ".png?v="
                  );
                  break;
                case "swatch_color_display_type_image_product":
                  ...
                  if (sImageIndex != "") {
                    bgImageSrc = Utils.optimizeImage(
                      data.images[sImageIndex],
                      bgSize
                    );
                  }
  ...
  function eventColorSwatches(event) {
    jQ(".boost-pfs-filter-item-swatch li span").each(function () {
      var img = jQ(this)
        .parents(".boost-pfs-filter-product-item-inner")
        .find(".boost-pfs-filter-product-item-main-image");
      if (event == "hover") {
  ...
  function buildReview(data) {
    var html = "";
    if (
      boostPFSThemeConfig.custom.hasOwnProperty("show_product_review") &&
  ...
  ProductSorting.prototype.compileTemplate = function () {
    var html = "";
    if (
      boostPFSThemeConfig.custom.show_sorting &&
      boostPFSTemplate.hasOwnProperty("sortingHtml")
    ) {
      ...
      if (sortingArr) {
        ...
        html = boostPFSTemplate.sortingHtml.replace(
          /{{sortingItems}}/g,
          sortingItemsHtml
        );
      }
  ...
  ProductSorting.prototype.render = function () {
    ...
    if (
      jQ(".boost-pfs-filter-custom-sorting").hasClass(
        "boost-pfs-filter-sort-active"
      )
    ) {
      jQ(".boost-pfs-filter-custom-sorting").toggleClass(
        "boost-pfs-filter-sort-active"
      );
    }
  ...
  ProductSorting.prototype.bindEvents = function () {
    ...
    jQ(".boost-pfs-filter-custom-sorting > button").click(function () {
      if (!jQ(".boost-pfs-filter-filter-dropdown").is(":animated")) {
        jQ(".boost-pfs-filter-filter-dropdown")
          .toggle()
          .parent()
          .toggleClass("boost-pfs-filter-sort-active");
      }
    });
  ...
  ProductDisplayType.prototype.compileTemplate = function () {
    var itemHtml =
      "<span>" + boostPFSThemeConfig.label.toolbar_viewas + "</span>";
    if (boostPFSThemeConfig.custom.product_item_type == "grid") {
      itemHtml +=
        '<a href="' +
        Utils.buildToolbarLink("display", "list", "grid") +
        '" title="Grid view" class="{{class.productDisplayType}}-item {{class.productDisplayType}}-grid" data-view="grid"><span class="icon-fallback-text"></span></a>';
    } else {
      itemHtml +=
        '<a href="' +
        Utils.buildToolbarLink("display", "list", "grid") +
  ...
  ProductLimit.prototype.compileTemplate = function () {
    var html = "";
    if (
      boostPFSThemeConfig.custom.show_limit &&
      boostPFSTemplate.hasOwnProperty("showLimitHtml")
    ) {
      ...
      if (numberList != "") {
        ...
        html = boostPFSTemplate.showLimitHtml.replace(
          /{{showLimitItems}}/g,
          showLimitItemsHtml
        );
      }
  ...
  Breadcrumb.prototype.compileTemplate = function (colData) {
    ...
    if (boostPFSTemplate.hasOwnProperty("breadcrumbHtml")) {
      if (
        ...
        if (
          typeof this.collectionTags !== "undefined" &&
          this.collectionTags !== null
        ) {
          breadcrumbItemsHtml += boostPFSTemplate.breadcrumbItemLink
            .replace(/{{itemLink}}/g, "/collections/" + colInfo.handle)
            .replace(/{{itemTitle}}/g, colInfo.title);
          breadcrumbItemsHtml += " {{breadcrumbDivider}} ";
          breadcrumbItemsHtml +=
            boostPFSTemplate.breadcrumbItemSelected.replace(
              /{{itemTitle}}/g,
              this.collectionTags[0]
            );
        } else {
          breadcrumbItemsHtml +=
            boostPFSTemplate.breadcrumbItemSelected.replace(
        ...
        breadcrumbItemsHtml += boostPFSTemplate.breadcrumbItemSelected.replace(
          /{{itemTitle}}/g,
          Settings.getSettingValue("label.defaultCollectionHeader")
        );
      }
    }
  ...
  Filter.prototype.afterRender = function (data) {
    ...
    var totalProduct =
      data.total_product +
      "<span> " +
      boostPFSThemeConfig.label.items_with_count_other +
      "</span>";
    if (data.total_product == 1) {
      totalProduct =
        data.total_product +
        "<span> " +
        boostPFSThemeConfig.label.items_with_count_one +
        "</span>";
    }
    ...
    if (isiOS) {
      Utils.isMobile() &&
        jQ(".boost-pfs-filter-product-item")
          .find("a")
    ...
    if (boostPFSThemeConfig.custom.swatch_color_event_change_image != "none") {
      eventColorSwatches(
        boostPFSThemeConfig.custom.swatch_color_event_change_image
      );
    }

    if (typeof boostRemoveImageLoadingAnimation == "function") {
      boostRemoveImageLoadingAnimation(
        jQ(Selector.products).find("[data-boost-image-loading-animation]")
      );
    }
  };
  ...
  ProductListPlaceholder.prototype.render = function () {
    ...
    var placeholderLimit =
      this.settings.productsPerRow || this.settings.placeholderProductPerRow;
  ...
  function equalHeight(data) {
    ...
    for (var i = 0; i < equal_els.length; i++) {
      ...
      for (var j = 0; j < equal_els[i].length; j++) {
        ...
        var height = jQ(item)
          .find(".boost-pfs-filter-product-bottom-inner")
          .height();
    ...
    if (equal_height != "false") {
      var cropImagePosition =
        boostPFSFilterConfig.general.cropImagePossitionEqualHeight;
      ...
      if (equal_height != "false" && equal_height != "auto") {
        ...
        var ratioWidthHeightOther =
          boostPFSThemeConfig.custom.ratio_width_height_other;
        if (ratioWidthHeightOther == "" && ratioWidthHeight == "") {
          ...
          if (ratioWidthHeightOther == "") {
            ...
            ratioWidthHeight =
              parseInt(ratioWidthHeight[1]) / parseInt(ratioWidthHeight[0]);
            heightImage = widthImage * ratioWidthHeight;
          } else if (ratioWidthHeightOther != "") {
            ratioWidthHeightOther = ratioWidthHeightOther.split(":");
            ratioWidthHeightOther =
              parseInt(ratioWidthHeightOther[1]) /
              parseInt(ratioWidthHeightOther[0]);
            heightImage = widthImage * ratioWidthHeightOther;
          }
        }
        jQ(".boost-pfs-filter-product-item-image-link")
          .css({ "padding-top": heightImage + "px" })
          .addClass(
            "boost-pfs-filter-crop-image-position-" + cropImagePosition
          );
      }
    }
  }
